    ...
    try {
        ...
        async function generateEmbedHTML(contentData) {
            ...
            if (link && link.startsWith('https://dramaspots.com/music/')) {
                const html = await getMusic(link) ; // <-- await the promise
                return html;
            }
...
<picture>
  ...
  <img src="${article.image}" alt="${article.title}" class="article-img-main shadow">
</picture>
  ...
  <img src="${a.image}" alt="Trending" class="related-article-img me-3">
</picture>
                                <span>${a.title}</span>
                            </a>
                        </li>
