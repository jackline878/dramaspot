...
const TARGET_URL = "https://moviemax-b8cf.onrender.com"; //
const TARGET_URL2 = "https://wustream.onrender.com/"; //
// === STATIC FILES ===
...
app.use('/api/celebrities', require('./routes/celebrity'));
...
if (TARGET_URL2) {
    setInterval(() => {
        fetch(`${TARGET_URL2}`)
            .then(res => res.text())
            .then(body => console.log(`✅ Pinged ${TARGET_URL2} — ${body}`))
            .catch(err => console.error(`❌ Failed to ping ${TARGET_URL2}: ${err.message}`));
    }, 780000); // 13 minutes in milliseconds
}
