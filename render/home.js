...
<section class="hero-section hide">
    <div class="mx-2">
        <p>Your #1 source for the latest celebrity gossip, news, and trends. Stay in the loop with exclusive stories, interviews, and more!</p>
        <div class="category-boxes">
            ${data.categories.slice(0, 6).map((c, i) => {
        // Pick an icon for each category (fallback to fa-star)
        const icons = [
            "fa-star", "fa-film", "fa-music", "fa-heart", "fa-fire", "fa-user-friends"
        ];
        const icon = icons[i % icons.length];
        return `<a href="/category/${encodeURIComponent(c.name)}" class="category-box text-decoration-none">
                        <div class="category-title">${c.name}</div>
                    </a>`;
    }).join('')
        }
        </div>
        <a href="#latest-posts" class="btn btn-pink mt-1">See Latest Gossip <i class="fas fa-arrow-down"></i></a>
    </div>
</section>
        ...
        <div class="m-0 all">
            ...
            <h1 class="mt-3">Featured</h1>
                ...
                <div class="card mb-4 post-card shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="mb-2">
                        ${data.featured.subcategories.map((s, i) => `<a class="text-decoration-none" href="/category/${s.category.name}"><span class="badge m-1 bg-${colors[s.category.id % 6]} text-light category-badge">${s.category.name}</span></a>`).join('')
                         }
                        </div>
