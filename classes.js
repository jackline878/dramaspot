...
class GetArticle {
    ...
    async getArticle() {
        const articlePromise = Article.findOne({
            where: { slug: this.slug },
            include: [
                { model: User, as: 'author', attributes: ['id', 'username', 'email', 'profile_pic', 'role'] },
                {
                    model: Subcategory,
                    as: 'subcategories',
                    attributes: ['id', 'name'],
                ...
                { model: Hashtag, as: 'hashtags', attributes: ['id', 'name'] },
            ...
            order: [
                [{ model: ArticleContent, as: 'contents' }, 'order', 'ASC'],
        ...
        const [articleData, viewed] = await Promise.all([
            articlePromise,
            // this.userId ? UserInteraction.findAll({
            //     where: { user_id: parseInt(this.userId), interaction_type: 'view' },
        ...
        let article = articleData.get({ plain: true });
        ...
        // article.isLiked = this.userId ? await IsLiked.isLiked(article.id, 'Article', this.userId) : false;
        article.likesCount = article.likes?.length || 0;

        // Get comments in parallel
        //  const commentsPromise = AllComments.getAllComments(article.id, 1, 10, this.userId);

        const viewedIds = viewed.map(v => v.article_id);

        // Extract category + hashtag IDs for later queries
        const categoryCount = {};
        const tagCount = {};
        const categories = [];
        ...
        const [
            ...
            topTags.length ? Article.findAll({
                where: {
                    status: 'published',
                    id: { [Op.notIn]: [article.id, ...viewedIds] }
                },
                include: [
                    { model: Hashtag, as: 'hashtags', attributes: ['id'], where: { id: topTags }, through: { attributes: [] } },
                    {
                        model: Subcategory,
                        as: 'subcategories',
                        attributes: ['id', 'name'],
            ...
            UserInteraction.findAll({
                attributes: ['article_id', [Sequelize.fn('COUNT', Sequelize.col('article_id')), 'view_count']],
                where: { interaction_type: 'view', createdAt: { [Op.gte]: Sequelize.literal('NOW() - INTERVAL 31 DAY') } },
                group: ['article_id'],
            ...
            topCategories.length ? Article.findAll({
                where: {
                    status: 'published',
                    id: { [Op.notIn]: [article.id, ...viewedIds] }
                },
        ...
        // Get most viewed based on interactions
        const articleIds = interactions.map(i => i.article_id);
        const mostViewed = articleIds.length
            ? await Article.findAll({
                where: {
                    id: { [Op.in]: articleIds, [Op.notIn]: [article.id, ...viewedIds] }
                },
        ...
        return {
            ...
            mostViewed,
            recommended: [],
            related: [...HashtagArticles, ...categoryArticles].slice(0, 10)
        };
    }

    static async getArticle(slug) {
        const instance = new GetArticle(slug);
