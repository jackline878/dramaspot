...
async function findUser(req, res) {
    try {
        ...
        // Filtered article count by status
        ...
        const isFollowing = user.Followers?.some(f => parseInt(f.follower_id) === currentUserId) || false;
        const isFollower = user.Followings?.some(f => parseInt(f.followed_id) === currentUserId) || false;
...
async function show(req, res) {
    try {
        ...
        // Count only articles with matching status
        const totalArticles = await models.Article.count({
            where: { userId: id, status },
        });
        ...
        const result = await models.User.findByPk(id, {
            include: [
                { model: models.Follower, as: 'Followers' },
                { model: models.Follower, as: 'Followings' },
                {
                    model: models.Article,
                    as: 'articles',
                    where: { status },
                    required: false, // allow user even with no matching articles
                    limit,
                    offset,
                    order: [['createdAt', 'DESC']],
                }
