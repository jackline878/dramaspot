...
exports.getLatestArticlesPagxe = async (req, res) => {
...
exports.getLatestArticlesPage = async (req, res) => {
  const page = parseInt(req.query.page || 2); // default to page 2
  const limit = parseInt(req.query.limit || 13);
  const offset = (page - 1) * limit;

  try {
    // 1. Collect IDs from featured, mostViewed, personalized, categories (page 1 sections)
    ...
    const mostViewedIds = await UserInteraction.findAll({
      attributes: ['article_id'],
      where: {
        interaction_type: 'view',
        createdAt: {
    ...
    const personalized = req.userData?.userId
      ? await getPersonalizedArticleIds(req.userData.userId)
      : [];

    const categoryArticleIds = await getCategoryArticleIds();

    const excludeIds = [
      ...featured.map(a => a.id),
      // ...mostViewedIds.map(v => v.article_id),
      // ...personalized,
    ];

    // 2. Fetch latest excluding those
    ...
    console.error('Error fetching latest articles page:', error);
    res.status(500).json({ message: 'Internal server error' });
  }
};

async function getPersonalizedArticleIds(user_id) {
  const viewed = await UserInteraction.findAll({
    attributes: ['article_id'],
    where: {
      user_id,
  ...
  return viewed.map(v => v.article_id);
}

async function getCategoryArticleIds() {
...
exports.getHomePageContentf = async (req, res, next) => {
...
exports.getHomePageContent = async (req, res, next) => {
  try {
    const user_id = req.userData?.userId || null;
    ...
    const [
      ...
      Article.findAll({
        ...
        limit: 16 // 3 featured + 10 latest, filter later
      }),
      ...
      UserInteraction.findAll({
        ...
        where: {
          interaction_type: 'view',
          createdAt: { [Op.gte]: Sequelize.literal('NOW() - INTERVAL 1 DAY') }
        },
        group: ['article_id'],
        order: [[Sequelize.literal('view_count'), 'DESC']],
        limit: 10,
      ...
      Comment.findAll({
        ...
        where: { createdAt: { [Op.gte]: Sequelize.literal('NOW() - INTERVAL 10 DAY') } },
        group: ['article_id'],
        order: [[Sequelize.literal('comment_count'), 'DESC']],
        limit: 10,
        raw: true
      ...
      Hashtag.findAll({ limit: 6, attributes: ['id', 'name'] }),
      ...
      Category.findAll({
        ...
        include: [{
          ...
          as: 'subcategories',
          required: true,
          include: [{ model: Article, as: 'articles', required: true, attributes: [] }]
        }],
        limit: 6,
        distinct: true
      }),
    ...
    // Filter latest to exclude featured
    const featuredIds = new Set(featured.map(f => f.id));
    const latestFiltered = latest.filter(a => !featuredIds.has(a.id)).slice(0, 10);

    // Most viewed articles
    const mostViewedIds = new Set(interactions.map(i => i.article_id));
    const mostViewed = await Article.findAll({
      where: {
        id: [...mostViewedIds].filter(id => !featuredIds.has(id) && !latestFiltered.some(l => l.id === id)),
        status: 'published'
      },
      attributes: ['id', 'title', 'slug', 'image', 'published_at']
    });

    // Prepare category articles limited to 10
    const categoriesWithLimitedArticles = categoriesAll.map(cat => {
      const allArticles = cat.subcategories.flatMap(sub => sub.articles || []);
      const uniqueArticles = [...new Map(allArticles.map(a => [a.id, a])).values()]
        .sort((a, b) => new Date(b.published_at) - new Date(a.published_at))
        .slice(0, 10);
      return { id: cat.id, name: cat.name, articles: uniqueArticles };
    });
    ...
    if (user_id) {
      const viewed = await UserInteraction.findAll({
        ...
        where: { user_id, interaction_type: 'view' },
        group: ['article_id'],
        order: [[Sequelize.literal('views'), 'DESC']],
        limit: 20,
        raw: true
      ...
      const viewedIds = new Set(viewed.map(v => v.article_id));
      personalized = await Article.findAll({
        attributes: ['id', 'title', 'slug', 'image', 'published_at'],
        where: {
          status: 'published',
          id: { [Op.notIn]: [...featuredIds, ...latestFiltered.map(l => l.id), ...viewedIds, ...mostViewedIds] }
        },
        order: [['published_at', 'DESC']],
        limit: 10
      });
    ...
    // API or server-rendered response
    const payload = {
      featured,
